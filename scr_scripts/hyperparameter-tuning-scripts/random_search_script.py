...
class TrainUsingRandomSearch:
    def __init__ (self, standard_randomsearch_parameters, n_iter=10000):
    ...
    def _distribute_attributes_and_predictions (self, test_dataset_x, convert_cv_to_pandas):
        ...
        if convert_cv_to_pandas:
            attributes_to_store.append(("rs_cv_results", pd.DataFrame(self.rs_global_instance.cv_results_)))
        else:
    ...
    def start_randomsearch_training (self, train_dataset_x, train_dataset_y, test_dataset_x):
        if not all(dataset == None for dataset in [train_dataset_x, train_dataset_y, test_dataset_x, test_dataset_y]): 
